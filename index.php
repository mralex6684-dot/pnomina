...
// Redirigir empleados a su portal
if (esUsuarioNormal()) {
    header('Location: ' . APP_URL . '/mi_dashboard.php');
    exit;
}
...
<div class="stats-grid">
  <div class="stat-card">
    <div><div class="stat-label">Total Empleados</div><div class="stat-value"><?= $totalEmpleados ?></div></div>
  ...
  <div class="stat-card">
    <div><div class="stat-label">Nóminas Pendientes</div><div class="stat-value"><?= $nominasPendientes ?></div></div>
    <div class="stat-icon yellow"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div>
  </div>
  <div class="stat-card">
    <div><div class="stat-label">Total Pagado Este Mes</div><div class="stat-value cop"><?= formatCOP($totalPagadoMes) ?> COP</div></div>
  ...
  <div class="stat-card">
    <div><div class="stat-label">Horas Extra Registradas</div><div class="stat-value"><?= $horasExtra ?></div></div>
...
<div class="dashboard-grid">
  <div class="card">
    <div class="card-header">
      ...
      <a href="<?= APP_URL ?>/modules/nomina/generar.php" class="btn btn-secondary btn-sm">Ver todas</a>
      <?php endif; ?>
    </div>
    <div class="card-body table-wrap">
      ...
      <?php else: ?>
      <table>
        <thead><tr><th>Empleado</th><th>Cargo</th><th>Mes</th><th>Total</th><th>Estado</th></tr></thead>
        <tbody>
          <?php
          ...
          foreach ($ultimasNominas as $n): $est = $n['estado']; ?>
          <tr>
            ...
            <td><span class="badge <?= $badges[$est] ?? 'badge-blue' ?>"><?= $labels[$est] ?? $est ?></span></td>
          </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="alerts-list">
        <?php if ($sinPagoMes > 0): ?>
        <div class="alert-item"><?= $sinPagoMes ?> empleado<?= $sinPagoMes > 1 ? 's' : '' ?> sin pago del mes actual</div>
        <?php endif; ?>
        <?php if ($nominasPendientes > 0): ?>
        <div class="alert-item yellow"><?= $nominasPendientes ?> nómina<?= $nominasPendientes > 1 ? 's' : '' ?> pendiente<?= $nominasPendientes > 1 ? 's' : '' ?> de aprobación</div>
        <?php endif; ?>
        <div class="alert-item green">Reporte mensual listo para generar</div>
        <div class="alert-item yellow">Verificar horas extra del período actual</div>
      </div>
    </div>
  </div>
</div>
